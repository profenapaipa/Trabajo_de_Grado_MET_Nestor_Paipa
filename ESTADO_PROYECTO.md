...
| 8 | Nota de alcance preexperimental (Metodología/Resultados) | ⏳ | Ninguna | Autorizar redacción (insumos nuevos — ver nota 2026-09-01) |
...
| 10 | Corrección del lenguaje de autonomía (Mago de Oz) | 🟡 PARCIAL | Fase 9 ✅; las 5 ocurrencias restantes dependen de la Fase 12 | Esperar resolución de la Fase 12 (criterio de vocabulario ya definido — ver nota 2026-09-01) |
| 11 | Unificación de valores PAUSAR/PENSAR/ACTUAR (fuente de verdad = `App.tsx`) | ⏳ | Fase 9; la parte dentro del Guión técnico depende de la Fase 12 | Autorizar sub-zona E1 (Propuesta estructural) (criterios de activación ya definidos — ver nota 2026-09-01) |
| 12 | Relación entre los dos guiones experimentales (verbal/gestual vs. técnico de cubos) | 🔴 BLOQUEADA | Decisión metodológica del autor | El autor define si son condiciones comparativas, protocolo integrado, o versiones sucesivas (ver nota 2026-09-01) |
| 13 | Metodología (Participantes/Procedimiento en modo prospectivo) | ⏳ | Fases 8-12 | — (bitácora y roles ya definidos — ver nota 2026-09-01) |
...
## Corrección — El buzzer del cubo ya no se describe como "pendiente de integración futura"

*Registrado 2026-09-02, a partir de una revisión del autor sobre `main.tex`. Commit pendiente.*

- **Problema**: `main.tex` contenía dos framings contradictorios sobre el sonido. Una explicación ya correcta (línea ~2368, subsección "Descripción técnica de los cubos inteligentes") decía que el buzzer nunca se conectó y que el sonido se resuelve por completo en el navegador del docente. Pero 2 diagramas TikZ, 1 tabla y varios párrafos alrededor seguían describiendo el buzzer como "tercer actuador multisensorial" cuya integración "se completará en una fase posterior de desarrollo del firmware" — tratándolo como una pieza pendiente del mismo sistema, no como una solución ya cerrada y definitiva por otra vía.
- **Corrección** (11 puntos en `main.tex`): (1) párrafo de aportes propios (línea ~2366) — los tonos ya no se describen como "a la espera del buzzer", sino como implementados vía navegador; (2)-(3) los dos párrafos de hardware del cubo (líneas ~2379-2394 y ~2405-2419) — "tres actuadores" pasa a "dos" (LED + vibración), y el buzzer se describe como componente físico sin conexión eléctrica activa; (4) Figura 4.6 (arquitectura interna del cubo) — el nodo Buzzer queda con trazo discontinuo y la etiqueta "sin conectar"; (5) párrafo "comportamiento observable del cubo" (línea ~2458) — ya no dice "cuando se complete la integración del firmware, su sonido"; (6) Figura 4.7 (arquitectura general) — se retira el nodo Buzzer conectado a los cubos (que sugería una vía funcional inexistente) y se agrega un nodo nuevo, en trazo discontinuo, junto al Frontend, mostrando que el sonido PPA lo emite el navegador del docente, no el canal WiFi/TCP 3333; (7) Figura 4.8 (flujo de datos) — el recuadro lateral del componente sonoro ya no dice "se completará en una fase posterior", sino que el campo de sonido es un remanente sin uso, porque el sonido real ya se emitió antes en el navegador; (8) párrafo posterior a esa figura; (9)-(10) Tabla 4.3 (componentes del sistema) — filas de Cubo, Buzzer y Frontend corregidas; (11) párrafo de Justificación (línea ~475) — "se materializa en el cubo mediante tres canales" pasa a distinguir que solo luz y vibración ocurren en el cubo, mientras el sonido lo aporta el computador del docente en sincronía.
- **Validación**: compilado 2 veces con `pdflatex -halt-on-error` (0 errores fatales, 94 páginas ambas pasadas); revisión visual directa de las 5 páginas afectadas (22, 69, 70, 71, 72) confirmando que los 2 diagramas TikZ modificados y el nuevo nodo renderizan correctamente, sin desbordar la página. La advertencia preexistente "Label `fig:placeholder' multiply defined" sigue presente (10 ocurrencias, ya registrada en `PENDIENTES_TESIS.md`) y no fue introducida por esta corrección — ninguno de los 4 labels nuevos/editados (`fig:cubo-arquitectura`, `fig:arquitectura-general`, `fig:flujo-datos-accion`, `tab:componentes-sistema`) aparece en esa lista de advertencias.
- **Impacto en el proyecto**: ninguno sobre el hardware o el firmware reales — es una corrección de consistencia documental. No cierra ni reabre ninguna fase de la tabla general; el pendiente de firmware "Integrar el comando sonoro en el software/firmware del cubo" (`PENDIENTES_TESIS.md`, sección Prototipo) permanece igual, pero ahora está correctamente enmarcado como una posibilidad futura y no como una promesa de "fase posterior" ya en curso.

## Tarea técnica — Simulador de práctica, Control Mago de Oz reforzado y corrección del motor de reglas

*Registrado 2026-09-02. Tarea independiente de la tesis y de cualquier decisión metodológica — no corresponde a ninguna fase numerada. Commit `ec5ac6c`.*

- **Reestructuración**: el frontend (`front-juego-acacia`) queda organizado en 3 pestañas principales: (1) Control Mago de Oz (hardware real, sin cambios de fondo en su función), (2) Simulación (nueva — Tutorial guiado / Simulación libre / Control simulado, sin cubos físicos, para practicar sin depender del hardware), (3) Vista de observador (nueva — réplica de solo lectura de los cubos de Control Mago de Oz, con bitácora de notas). La Vista de observador estaba anidada dentro de Simulación en un diseño anterior; se movió a pestaña principal por instrucción explícita del autor.
- **Motor de reglas compartido** (`laEscaleraRules.ts`): Simulación libre, Tutorial guiado y Control simulado reutilizan las mismas funciones (`legalMovesFor`, `applyMove`, `computeWinBoard`, `boardsEqual`, `isStuck`); Control Mago de Oz reutiliza `legalMovesFor` para detección de fallas reales y, desde este commit, también `computeWinBoard`/`boardsEqual`/`isStuck` para victoria/derrota.
- **Bug corregido — tablero meta irreconocible para 2+ pares**: ver el detalle completo en `PENDIENTES_TESIS.md` (tabla "Prototipo"). En resumen: `computeWinBoard` ponía la casilla vacía en el extremo del tablero en vez de en el centro (su posición original), lo que hacía que el estado "meta" calculado fuera *matemáticamente inalcanzable* por las reglas del juego para 2 o más pares — todo ejercicio con 2+ pares terminaba en "derrota" sin importar qué tan bien jugara el sujeto. Corregido y verificado por BFS exhaustivo (n=1..5, alcanzable + terminal + coincide con la fórmula conocida n²+2n de movimientos mínimos) y con partidas reales vía Playwright.
- **Mago de Oz reforzado sin agregar automatismos**: señal sonora y patrón de vibración diferenciados por fase (Pausar/Pensar/Actuar), medidores de "condición acumulada" por fase (informativos, nunca disparan nada solos — coherente con la regla vinculante de vocabulario ya registrada en `DECISIONES_PROYECTO.md`), música ambiental configurable, bitácoras independientes de eventos de cubos vs. decisiones del operador.
- **Validación**: `npm run build` (`tsc -b && vite build`) limpio; BFS exhaustivo del motor de reglas para n=1..5; partidas jugadas de extremo a extremo vía Playwright (pares=1 y pares=2) contra la interfaz real, confirmando el banner de victoria y el conteo de movimientos.
- **Resultado**: simulación de práctica funcional para los 5 niveles de pares (antes solo 1 par funcionaba correctamente); Control Mago de Oz gana detección de victoria/derrota reutilizando el mismo motor ya corregido.
- **Sin verificar contra hardware real** (ninguno conectado en esta sesión): la detección de fallas reales y la nueva detección de victoria/derrota en Control Mago de Oz están verificadas por lógica y tipos, no contra los cubos físicos. **Actualización 2026-09-02**: la detección de fallas ya implementa el segundo paso del algoritmo Levantado/Reubicado/No detectado (`DECISIONES_PROYECTO.md`) — antes de contar una falla, consulta `esclavosRef.current` (última lista de `esclavosConectados`) y, si el cubo implicado no aparece conectado, registra el evento como `cubo_no_detectado` en vez de `falla_movimiento`, sin sumar al medidor de Pausar/Pensar. Sigue pendiente de prueba con hardware real — ver `PENDIENTES_TESIS.md`.
- **Impacto en el proyecto**: ninguno sobre la tesis, la metodología, el backend o el firmware. No cierra ni reabre ninguna fase de la tabla general.
...
| `2f918ef` | Separa el controlador simulado del juego y agrega bitácoras independientes | 2026-09-01 |
| `ec5ac6c` | Reestructura el frontend en 3 pestañas (Control Mago de Oz, Simulación, Vista de observador) y corrige el tablero meta de La Escalera | 2026-09-01/02 |
...
**Nota (2026-09-01) — decisiones metodológicas incorporadas a `DECISIONES_PROYECTO.md` y su efecto sobre las fases abiertas:** esta sesión registró en `DECISIONES_PROYECTO.md` un conjunto de decisiones ya tomadas por el autor (encuadre metodológico, criterios de activación PPA, requisitos de bitácora, separación de roles operador/observador). Ninguna fase fue ejecutada ni cerrada en esta sesión — solo se documentaron decisiones para uso posterior. Ningún estado (✅/🟡/⏳/🔴) de la tabla cambia por esta nota; se deja constancia de qué fase gana insumos y cuál sigue exactamente igual:
- **Fase 8** (Nota de alcance preexperimental): gana insumos redaccionales concretos — diseño de caso único, enfoque mixto, sin grupo de contraste ni pretensión causal — pero su redacción sigue sin autorizar. La muestra quedó cerrada en 1 estudiante (confirmado por el autor, 2026-09-01) — ver `DECISIONES_PROYECTO.md`, sección Investigación.
- **Fase 10** (Corrección del lenguaje de autonomía): la nueva regla de vocabulario vinculante ("sugerencia"/"confirmación"; prohibidos "detecta", "decide", "activa automáticamente") es el criterio explícito contra el que deberá auditarse el contenido que reemplazó al antiguo Guión técnico, una vez se resuelva la discrepancia señalada arriba sobre el commit `799731e`. No se ejecutó ninguna auditoría en esta sesión; las 5 líneas seguían registradas como pendientes antes de este commit.
- **Fase 11** (Unificación de valores PAUSAR/PENSAR/ACTUAR): las decisiones nuevas fijan los criterios operacionales de activación (Pausar = 2 fallas consecutivas, Pensar = señal de confusión, Actuar = latencia relativa al nivel, calibrada en la sesión de familiarización) y exigen que vivan en configuración versionada, no en código. Esto añade a la Fase 11 el objetivo de unificar también **de dónde se leen** esos valores, no solo sus nombres. El umbral base de Actuar permanece sin asignar (pendiente explícito, sin inventar).
- **Fase 12** (Relación entre los dos guiones, bloqueada): el nuevo encuadre metodológico declara explícitamente "sin grupo de contraste ni pretensión causal", lo que podría tener relación con la pregunta 1 del Acta (¿diseño de dos sesiones comparadas o condición única?). Esto es una lectura de esta sesión, no una decisión explícita del autor sobre esa pregunta puntual — la Fase 12 se mantiene 🔴 BLOQUEADA y no se reinterpreta por inferencia.
- **Fase 13** (Metodología — Participantes/Procedimiento): los nuevos requisitos de bitácora y de separación de roles (operador/observador) son insumos directos para la sección de Procedimiento cuando se autorice. Los identificadores de las personas que ocuparán esos roles siguen sin asignar (pendiente explícito, no debe inventarse).
...
| 2026-09-01/02 | Tarea técnica (no asociada a una fase) | Simulador de práctica (Simulación: Tutorial guiado/Simulación libre/Control simulado) separado de Control Mago de Oz en 3 pestañas principales; Vista de observador movida a pestaña principal; motor de reglas de La Escalera compartido entre los 4 modos; corrección del bug de `computeWinBoard` (tablero meta inalcanzable para 2+ pares); Control Mago de Oz gana detección de victoria/derrota reutilizando ese motor ya corregido; señal sonora/vibración/medidores PPA diferenciados por fase; música ambiental y bitácoras independientes | ✅ `npm run build` limpio; motor de reglas verificado por BFS exhaustivo (n=1..5) y con partidas reales vía Playwright; sin verificar contra hardware físico (detección de fallas y victoria/derrota de Control Mago de Oz) | `ec5ac6c` |
