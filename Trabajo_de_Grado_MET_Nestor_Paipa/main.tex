...
\begin{itemize}
    ...
    Pausar-Pensar-Elegir con los principios 
...
(Pausar, Pensar, Elegir), que se materializa en el cubo mediante 
...
\begin{figure}[h]
    ...
    \includegraphics[width=1.5\linewidth, trim={0.2cm 0.5cm 0.5cm 0.5cm}, clip]{Metodología.pdf}
...
El sistema está compuesto por tres agentes funcionales: \textit{Pausar}, \textit{Pensar} y \textit{Elegir}. Cada agente interviene sobre dimensiones conductuales, cognitivas y emocionales.
...
box/.style={rectangle, draw, rounded corners, align=center, text width=3.2cm},
...
\node[box, below right=of system] (elegir) {Agente Elegir};
...
\node[box, below=of elegir] (emo_elegir) {
Estados Emocionales \\ (Elegir) \\
...
Authoritative & Pausar/Elegir & Azul intenso & 35\% & Pulso firme & Control estructural \\
...
Encouraging & Pensar/Elegir & Verde-turquesa & 70\% & Arpegio & Refuerzo positivo \\
Decisive & Elegir & Verde brillante & 80\% & Ascendente & Confirmación deliberada \\
...
\begin{enumerate}
    \item Detección de intento impulsivo y activación del agente Pausar.
    \item Estabilización cognitiva y activación del agente Pensar.
    \item Confirmación deliberada y activación del agente Elegir.
\end{enumerate}
...
\begin{figure}[h]
    \centering
    % Ajusta los valores de trim (en mm o cm) hasta que la imagen se vea bien
    \includegraphics[width=1.5\linewidth, trim={0.2cm 0.5cm 0.5cm 0.5cm}, clip]{Juego Inteligente.png}
    \caption{Juego Inteligente xxxx}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}[h]
    \centering
    % Ajusta los valores de trim (en mm o cm) hasta que la imagen se vea bien
    \includegraphics[width=1.5\linewidth, trim={0.2cm 0.5cm 0.5cm 0.5cm}, clip]{vistascubo.png}
    \caption{Juego Inteligente xxxx}
    \label{fig:placeholder}
\end{figure}
%%% Explicar las vistas de la figura anterior.
%%%%%% ******* %%%%%%% Diagrama Electrónico
El núcleo de procesamiento de cada cubo es un microcontrolador ESP32,
que se conecta por WiFi como estación a la red inalámbrica generada por
el \textit{Master} para recibir sus instrucciones. Los periféricos de salida
conectados al microcontrolador son tres: un diodo LED de tipo RGB capaz
de producir cualquier combinación de color mediante la modulación
independiente de sus tres canales (rojo, verde y azul, con valores entre
0 y 255 en cada canal), un motor de vibración de tipo moneda con
regulación de intensidad por modulación de ancho de pulso (PWM), y un
altavoz piezoeléctrico o buzzer con capacidad de emitir tonos de
frecuencia y duración controladas programáticamente. En la implementación
actual, el LED y el motor de vibración reciben sus parámetros de
operación a través del mensaje enviado por el \textit{Master}; el
componente sonoro está definido a nivel de interfaz, pero el protocolo de
comunicación entre el \textit{Master} y el cubo todavía no incorpora un
campo para transportarlo, por lo que la reproducción física del buzzer a
partir de una instrucción remota no puede darse por demostrada de extremo
a extremo en el estado actual del desarrollo. La alimentación del
cubo se realiza mediante batería de litio recargable de [X] mAh,
...
un microcontrolador ESP32 —el mismo tipo de microcontrolador empleado en
los cubos individuales—, que opera de forma simultánea en dos roles de
red: como punto de acceso WiFi propio, al que se conectan los cubos, y
...
(\textit{Pausar}, \textit{Pensar} o \textit{Elegir/Actuar}), el estado
