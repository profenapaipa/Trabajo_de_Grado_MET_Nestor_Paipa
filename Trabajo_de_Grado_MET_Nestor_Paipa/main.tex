...
del juego La Escalera en un niño con Trastorno
...
Al ser un estudio exploratorio de caso único con un
solo participante, su aporte no es la generalización
...
El punto de partida tecnológico de esta investigación es el
prototipo físico de cubos inteligentes desarrollado por el
módulo INNOVA del proyecto ACACIA (Red CADEP ACACIA,
cofinanciado por el programa Erasmus+), un proyecto de
cooperación entre la Universidad NOVA de Lisboa y la
Universidad Distrital Francisco José de Caldas
\citep{ACACIA2025}. La subsección «Descripción técnica de los
cubos inteligentes» distingue, con evidencia verificable y no
solo por declaración, qué corresponde a esa entrega original y
qué es aporte propio de la presente investigación; en síntesis,
lo propio es el mecanismo de descubrimiento automático de la
red, el seguimiento del estado de conexión del sistema, el
protocolo de interacción Mago de Oz con sus criterios
operacionales de activación del ciclo Pausar-Pensar-Actuar, y
la adaptación completa del sistema al estudio del control
inhibitorio en un niño con TEA nivel 1 mediante EEG.
...
juego La Escalera en un niño con
...
\begin{itemize}
    ...
    inhibitorio en un niño con Trastorno
    ...
    un niño con Trastorno del Espectro
...
\subsection*{Propuesta de ajuste de objetivos (pendiente de aprobación del tutor)}
...
\begin{quote}
\textit{Propuesta de objetivo específico 2:} ``Implementar computacionalmente una metodología de análisis de señales EEG basada en potencia relativa alfa por canal (Alpha TRP), que permita comparar la actividad cortical entre los ejercicios de complejidad creciente (de uno a cinco cubos por lado) durante la aplicación del ciclo Pausar-Pensar-Actuar.''
\end{quote}
...
(Pausar, Pensar, Actuar), que se materializa mediante
tres canales de salida simultáneos: luz LED de color específico y
vibración háptica de patrón regulado, ambas en el propio cubo, e
indicación sonora de frecuencia predefinida, emitida por el
computador del docente en sincronía con las dos anteriores.
...
El antecedente más lejano de La Escalera es un juego de peones
de solitario dado a conocer por Édouard Lucas en 1883, en su
obra \textit{Récréations Mathématiques}, tal como documentan
Liévano Chaparro y Molina Monguí \citep{LievanoMolina2022}.
En la Universidad Distrital Francisco José de Caldas, ese
juego de transformación fue retomado y adaptado en distintos
trabajos de grado de la Maestría en Educación en Tecnología
\cite{Cardenas2017, RodriguezLopez2017, Natalia2017,
LievanoMolina2022}, entre los cuales Liévano Chaparro y Molina
Monguí desarrollaron la versión física de once posiciones
numeradas (Figura~\ref{fig:juego-peones-original}) sobre la que
se fundamenta el juego utilizado en la presente investigación.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{juego-peones-original.png}
    \caption{Versión física del juego La Escalera, de once posiciones numeradas (cinco fichas rojas y cinco azules con una posición central vacía), sobre la que se fundamenta el modelo matemático del espacio del problema. Fuente: \cite{LievanoMolina2022}.}
    \label{fig:juego-peones-original}
\end{figure}

\begin{figure}[h]
    \centering
        \caption{Reglas del juego La Escalera y comparación de las tres reglas de movimiento, tal como se presentan al jugador en la interfaz. Elaboración propia.}
...
la izquierda. No se permiten retrocesos \cite{PaezGonzalez2022, LievanoMolina2022}.
...
La versión digital del juego utilizada como referente para la
presente investigación fue programada originalmente en
Processing y migrada a p5.js para su publicación en línea por
Liévano Chaparro y Molina Monguí, junto con el modelo
computacional del espacio del problema y las métricas de
buclicidad y ramificación descritas en la
Sección~\ref{sec:fundamento_ecuaciones}
\cite{LievanoMolina2022}. En el capítulo de trabajos futuros de
esa investigación, los propios autores propusieron
explícitamente ``implementar el juego físico con conexión a la
interfaz gráfica, la base de datos, el sistema de análisis de
datos y el grafo como apoyo visual de las rutas y posibles
decisiones'' \citep{LievanoMolina2022}. La integración de los
cubos inteligentes del proyecto ACACIA con esa misma interfaz,
descrita en el Capítulo~4 de este documento, constituye una
respuesta directa a esa línea de trabajo futuro.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{interfaz-original-la-escalera.png}
    \caption{Interfaz gráfica original del juego La Escalera, programada en Processing y migrada a p5.js para su publicación en línea. Fuente: \cite{LievanoMolina2022}.}
    \label{fig:interfaz-original}
\end{figure}

\begin{figure}[h]
    \centering
        \caption{Espacio del problema del juego La Escalera: grafo no dirigido de los estados posibles del tablero, con el estado inicial, el estado meta y los puntos críticos señalados, generado siguiendo la metodología de graficación del espacio del problema propuesta por \cite{LievanoMolina2022}.}
\includegraphics[width=0.50\textwidth]{Grafo.png}
\label{fig:espacio-problema-propio}
\end{figure}
...
\begin{figure}[h]
    \centering
        \caption{Grafo del espacio del problema del juego La Escalera, generado computacionalmente con la metodología de \cite{LievanoMolina2022} (disposición de fuerzas entre nodos). El trazado en verde resalta una ruta de ejemplo dentro del grafo y el área en cian resalta una región de estados densamente conectados. Al tratarse de un modelo del espacio del problema y no del registro de una sesión real, no corresponde todavía a la trayectoria de un participante del estudio.}
\includegraphics[width=0.80\textwidth]{GJRTI.pdf}
\label{fig:grafo-fuerzas-propio}
\end{figure}
...
de herramientas con agentividad en el desempeño de un niño
...
y trabaja con un único participante con TEA nivel 1,
...
En el contexto de la presente investigación con el niño
...
\fbox{\parbox{0.82\textwidth}{%
    ...
    por intento ($B_i$) para el participante. Eje $x$: número
...
\fbox{\parbox{0.82\textwidth}{%
    ...
    por intento para el participante. Dispersión de $C_m$
...
\label{sec:diseno-investigacion}

El propósito educativo de la intervención es que el estudiante descubra y aplique la estrategia de medios y fines mediante un proceso de cuestionamiento permanente en cada movimiento del juego \textit{La Escalera}, al que en este documento se denomina ciclo \textbf{123}: qué opciones tiene disponibles, cuál de ellas elige como la mejor, y qué operador le permite pasar al estado que identificó como mejor.
...
\begin{figure}[H]
\centering
\definecolor{azulmarino}{HTML}{14375E}
\definecolor{mostaza}{HTML}{C4881E}
\begin{tikzpicture}[
    font=\footnotesize,
    node distance=0.25cm and 0cm,
    fase/.style={
        rectangle split,
        ...
        align=left,
        text width=10.4cm,
        rectangle split part fill={azulmarino, white, white}
    },
    arrow/.style={-{Latex}, thick, azulmarino}
]

\node[fase] (f1) {
    \textcolor{white}{\textbf{1. Selección del sujeto}}
    \nodepart{two} \textbf{Procedimiento:} verificación de los criterios de inclusión, diagnóstico de TEA nivel 1 confirmado y ausencia de contraindicación para el uso del casco EEG.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} un sujeto seleccionado.
};

\node[fase, below=of f1] (f2) {
    \textcolor{white}{\textbf{2. Consentimiento informado y tratamiento de datos}}
    \nodepart{two} \textbf{Procedimiento:} solicitud de autorización a los padres o tutores legales, con explicación del alcance del estudio y del tratamiento de los datos recolectados.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} consentimiento informado firmado.
};

\node[fase, below=of f2] (f3) {
    \textcolor{white}{\textbf{3. Alistamiento}}
    \nodepart{two} \textbf{Procedimiento:} verificación técnica de los cubos y de la base, calibración del casco OpenBCI, explicación del juego La Escalera y familiarización del estudiante con el entorno.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} lista de chequeo de condiciones técnicas y registro de familiarización.
};

\node[fase, below=of f3] (f4) {
    \textcolor{white}{\textbf{4. Implementación por niveles de complejidad}}
    \nodepart{two} \textbf{Procedimiento:} serie de ejercicios con el casco EEG puesto desde el inicio, escalando de uno a cinco cubos de cada color a cada lado del tablero, con el PPA sugerido por el sistema y confirmado por el docente.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} video, registro EEG continuo y bitácora de eventos por ejercicio.
};

\node[fase, below=of f4] (f5) {
    \textcolor{white}{\textbf{5. Entrevista semiestructurada}}
    \nodepart{two} \textbf{Procedimiento:} inmediatamente después de la sesión, identificación de los momentos principales del video y preguntas organizadas por categoría.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} grabación y transcripción de la entrevista.
};

\node[fase, below=of f5] (f6) {
    \textcolor{white}{\textbf{6. Procesamiento de datos}}
    \nodepart{two} \textbf{Procedimiento:} procesamiento del EEG en MATLAB según el protocolo Alpha TRP, y codificación del video y de la entrevista.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} mapas topográficos de potencia relativa alfa por ejercicio y matriz de codificación cualitativa.
};

\node[fase, below=of f6] (f7) {
    \textcolor{white}{\textbf{7. Análisis}}
    \nodepart{two} \textbf{Procedimiento:} análisis cruzado entre los mapas topográficos, la codificación de video y la entrevista, en función del efecto del PPA sobre el ciclo 123.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} hallazgos del análisis cruzado.
};

\node[fase, below=of f7] (f8) {
    \textcolor{white}{\textbf{8. Interpretación de resultados}}
    \nodepart{two} \textbf{Procedimiento:} interpretación de los hallazgos desde el marco teórico del efecto del PPA sobre el ciclo 123.
    \nodepart{three} \textbf{\textcolor{mostaza}{Producto:}} descripción del comportamiento observado en el caso estudiado.
};
...
\end{tikzpicture}
\caption{Diseño metodológico del estudio, organizado en fases con su procedimiento y producto asociado.}
\label{fig:diseno-metodologico}
\end{figure}
...
El presente estudio se desarrolla con un único participante: un niño con diagnóstico confirmado de Trastorno del Espectro Autista (TEA) nivel 1, bajo un diseño preexperimental, exploratorio, de caso único. Los criterios de inclusión son el diagnóstico de TEA nivel 1 confirmado por un profesional competente y la ausencia de contraindicación para el uso prolongado del casco EEG OpenBCI durante toda la sesión. La participación requiere el consentimiento informado firmado por los padres o tutores legales del niño, así como su autorización para el tratamiento de los datos de video, EEG y bitácora recolectados durante la sesión.
...
son dos: un diodo LED de tipo RGB capaz de producir cualquier
...
modulación de ancho de pulso (PWM). La placa incluye además un
altavoz piezoeléctrico o buzzer, pero sin ningún pin del
microcontrolador conectado a él ni instrucción de firmware que lo
accione, como se explicó arriba: el sistema integra su respuesta
multimodal completa —luz, vibración y sonido— combinando estos dos
actuadores físicos del cubo con el sonido que emite, en paralelo, el
computador del docente. La alimentación del cubo se realiza mediante batería
...
buzzer que la placa trae físicamente, pero que ningún pin del
microcontrolador acciona: el sonido del sistema no pasa por este
componente, sino que lo emite el computador del docente mediante el
navegador (ver la subsección anterior). Cada cubo
...
\node[box, below right=of cubo_esp32] (cubo_buz) {Buzzer \\ \footnotesize sin conectar};
...
\draw[arrow, dashed] (cubo_esp32) -- (cubo_buz);

\end{tikzpicture}
\caption{Arquitectura interna del cubo inteligente: un microcontrolador
ESP32-C3 SuperMini gestiona dos actuadores multisensoriales (LED
RGB y motor de vibración) y se alimenta de una batería Li-Po. La placa
incluye además un buzzer (trazo discontinuo) sin conexión eléctrica
activa al microcontrolador, por lo que no forma parte de la respuesta
multisensorial del sistema — el sonido lo emite en su lugar el
computador del docente, como se explica en el texto. El trazo delgado
hacia el acoplamiento magnético señala una relación
...
directamente sobre el LED y el motor de vibración.}
\label{fig:cubo-arquitectura}
\end{figure}

El comportamiento observable del cubo —su color y su patrón de
vibración— depende de la instrucción que recibe desde el ESP32
maestro; el componente sonoro de esa misma acción lo aporta, en
paralelo y sin pasar por el cubo, el computador del docente.
...
\node[box, below=of arch_cubos] (arch_ledvib) {LED RGB \\ Motor de vibración};
\node[box, right=of arch_frontend, text width=2.6cm, dashed] (arch_sonido) {Sonido PPA \\ \footnotesize navegador del docente, Web Audio API};
...
\draw[arrow, dashed] (arch_frontend) -- (arch_sonido);
...
que transmite los comandos de color y vibración, y
...
tablero se encuentra cada uno. El sonido de cada fase del PPA no viaja
por ninguno de estos dos canales (recuadro discontinuo): lo emite
directamente el navegador del docente mediante la API de audio del
navegador (Web Audio API), en sincronía con la orden enviada al cubo.}
...
\node[sidebox, right=of flow_recib, dashed] (flow_buzzer) {Componente sonoro \\ \footnotesize la información sonora llega hasta el maestro, pero no se usa: el sonido real ya se emitió antes, en el navegador del docente};
...
muestra que un campo de información sonora sigue viajando, sin
usarse, junto con el resto de la orden hasta el ESP32 maestro
—remanente de un diseño anterior—; el sonido real de cada fase ya se
resolvió antes de ese punto, en el navegador del docente, y no
depende de que este campo llegue a intervenir en el firmware del
cubo.}
...
directamente del mensaje que el maestro transmite al cubo; el campo
de información sonora que viaja en ese mismo mensaje no llega a
usarse, porque el sonido de cada fase ya lo emitió antes el navegador
del docente, como se indicó.
...
\hline
Frontend & Interfaz web mediante la cual el docente selecciona un cubo y una acción (Pausar, Pensar o Actuar); también emite el tono sonoro de cada fase mediante la API de audio del navegador & Implementado \\
\hline
...
\hline
Cubo (ESP32-C3 SuperMini) & Recibe la instrucción del maestro y activa sus actuadores de luz y vibración & Implementado (véase la fila del Buzzer para la aclaración sobre el sonido) \\
\hline
...
\hline
Buzzer & Componente físico presente en la placa del cubo, sin conexión eléctrica activa al microcontrolador ni uso en el firmware & No utilizado — el sonido del sistema lo emite en su lugar el computador del docente (navegador, Web Audio API) \\
\hline
...
al cubo correspondiente por el canal inalámbrico.
% [SUJETO A VERIFICACIÓN TÉCNICA]
La base debería además mantener un registro de los eventos de activación
(marca de tiempo, cubo activado, fase y estado emocional seleccionado), de
manera que ese registro pueda correlacionarse con los movimientos del juego
y con el archivo EEG. Este mecanismo de registro constituye un aporte
técnico relevante para el análisis cualitativo de la mediación docente
(identificar en qué puntos del espacio del problema el docente consideró
necesario intervenir), pero está pendiente de implementación y de
...
% [SUJETO A VERIFICACIÓN TÉCNICA]
Al finalizar la sesión, el sistema debería permitir exportar el registro
...
de cada evento, de modo que ese archivo pueda sincronizarse con el registro
de movimientos del juego y con la marca de inicio del registro EEG para el
análisis posterior. Esta función de exportación está pendiente de
implementación y de verificación en el prototipo.
...
\begin{enumerate}
    ...
    \item \textbf{Comparación entre canales}
        ...
        % Fin Código Imagen
\end{enumerate}
\subsection{Guion experimental: ciclo Pausar-Pensar-Actuar en La Escalera}

\textbf{Objetivo del guion:} caracterizar el efecto del ciclo Pausar-Pensar-Actuar (PPA) sobre el ciclo cognitivo 123 (qué opciones tengo, cuál elijo, qué operador me permite pasar al estado elegido) durante la resolución del juego \textit{La Escalera} por parte de un estudiante con TEA nivel 1.
...
\paragraph{Activación de Pausar}
Criterio operacional: se sugiere Pausar cuando el sistema detecta dos fallas consecutivas del estudiante en la aplicación del ciclo 123 (dos movimientos que no corresponden a una opción válida según las reglas del juego) o una señal de confusión observada por el docente. El docente confirma o descarta la sugerencia desde la consola.

\paragraph{Activación de Pensar}
Criterio operacional: el mismo que para Pausar (dos fallas consecutivas o señal de confusión), activado inmediatamente después de la fase Pausar dentro del mismo ciclo, con el propósito de sostener la atención del estudiante sobre las opciones disponibles antes de decidir.

\paragraph{Activación de Actuar}
% [PENDIENTE: CRITERIO DE ACTIVACIÓN DE ACTUAR — CONSULTAR CON EL DIRECTOR]
El criterio operacional para sugerir la activación de Actuar está pendiente de definición y se consultará con el director de tesis. No se anticipa aquí ningún criterio para no afirmar algo que todavía no ha sido acordado.

\paragraph{Secuencia operativa de un ejercicio}
Cada ejercicio, con una cantidad creciente de cubos por lado (ejercicios 1 a 5, véase la Sección~\ref{sec:diseno-investigacion}), sigue la misma secuencia: el estudiante observa el tablero y aplica el ciclo 123 de manera implícita mientras el docente observa el desarrollo del ejercicio; cuando se cumple el criterio de activación de una fase del PPA, la consola presenta la sugerencia junto con el motivo que la disparó; el docente confirma o descarta la sugerencia; si la confirma, el cubo correspondiente emite la señal multisensorial configurada para esa fase, según la configuración descrita en la sección «Propuesta estructural integradora»; el ejercicio continúa hasta que el estudiante completa el objetivo del juego o el docente decide finalizarlo.

\paragraph{Registro del ejercicio}
% [SUJETO A VERIFICACIÓN TÉCNICA]
...
\subsection{Sincronización de las fuentes de datos}

% [SUJETO A VERIFICACIÓN TÉCNICA]
Video, registro EEG y bitácora del sistema se sincronizan mediante una marca de inicio manual: un gesto físico visible en la grabación de video, ejecutado en el instante en que arrancan simultáneamente el registro EEG y la bitácora del sistema. A partir de ese instante cero, los tres registros se alinean por tiempo relativo.

\subsection{Distinción entre cubo levantado y cubo no detectado}

% [SUJETO A VERIFICACIÓN TÉCNICA]
Para efectos del análisis, un cambio de estado de un cubo se evalúa en dos pasos. Primero se verifica si es compatible con las reglas de movimiento del juego: en cada instante puede haber como máximo un cubo levantado, y ese cubo debe pertenecer al conjunto de cubos que pueden moverse legalmente, es decir, avanzar únicamente en el sentido opuesto al de inicio, sin retroceder, y saltando como máximo sobre un cubo. Si el cambio es compatible con un movimiento legal, se interpreta como cubo levantado. Si no lo es, no se asume de inmediato que el cubo se desconectó: se revisa, en segundo lugar, una señal independiente del canal de posición que confirme la desconexión, antes de excluir ese cambio de estado del análisis. Esta misma distinción determina cómo se representa cada cubo en la interfaz del docente.

\subsection{Objeto del análisis}
